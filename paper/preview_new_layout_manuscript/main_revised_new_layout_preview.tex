\documentclass[reprint,amsmath,amssymb,aps,prb,floatfix,showkeys,superscriptaddress]{revtex4-2}
...
\newcommand{\nonfloatcaption}[1]{\refstepcounter{figure}\par\smallskip\noindent\footnotesize\textbf{FIG.~\thefigure.} #1\par}
\makeatletter
\newenvironment{widetextnolines}{%
  \par\ignorespaces
  \onecolumngrid
  \vskip10\p@
  \prep@math@patch
...
the Painlev\'e--Gullstrand background onto an anisotropic $XY$ chain
with a bond-dependent Dzyaloshinskii--Moriya interaction and a transverse field:
\begin{align}
  H_{\mathrm{spin}}
  ={}&
  -\frac{1}{4\varepsilon}
...
\end{equation}
where $k\in[-\pi,\pi)$ is the dimensionless lattice momentum,
...
\end{equation}
and let $\epsilon\to0$ at fixed $q$, so that
$k=\epsilon q\to0$. The dispersion then becomes
\begin{equation}
  E_{\pm}(\epsilon q)
  =
  \beta q
  \pm |q|
  +O\!\left(\epsilon^{2}|q|^{3}\right).
...
Brillouin-zone edges $k=\pm\pi$, corresponding to conventional
fermion doublers. Their physical momenta are of order
$|q|\sim\pi/\epsilon$, even though their excitation energies remain
low. For $p>0$, the term $p(1-\cos k)$ lifts the zero-energy modes at
...
immediately to the right of $k=0$ but negative energy at
...
\begin{equation}
  ...
  \bigl(
    c_0,\ldots,c_{L-1},
    c_0^\dagger,\ldots,c_{L-1}^\dagger
  \bigr)^{\mathsf T}.
...
$k=0$, where the lattice dispersion approaches that of the
...
\begin{equation}
  H
  =
  \varepsilon\sum_{j=0}^{L-1}\mathcal{H}_j,
  \qquad
  \mathcal{H}_j
  =
  \mathcal{H}_j^+
...
Our interpretation of the horizon dynamics focuses on the interval
before appreciable wave-packet weight reaches the physical ends of the
chain. Some plotted intervals extend to later times in order to display
lattice collapse, reflection, or spectral evolution; these late-time
portions are not used for quantitative comparison with boundary-free
continuum trajectories.
...
\begin{figure*}[t]
\includegraphics[width=\linewidth]{figure/fig02_p0_aplus.pdf}
\iffalse
...
\label{fig:bh_p0}
\fi
\caption{\textit{Layout preview only: final caption to be supplied by the authors.}}
\label{fig:bh_p0}
\end{figure*}
\begin{figure*}[t]
\includegraphics[width=\linewidth]{figure/fig03_p0_aminus.pdf}
\caption{\textit{Layout preview only: final caption to be supplied by the authors.}}
...
To make weak-amplitude features visible without discarding their sign, we use a symmetric asinh mapping for the color scale in Fig.~\ref{fig:bh_p1}(a) and Fig.~\ref{fig:wh_p0}(a):
\begin{equation}
 C(\mathcal{H})
 =\frac{1}{2}\left[
 1+
 \frac{\operatorname{asinh}(\mathcal{H}/\mathcal{H}_0)}
 {\operatorname{asinh}(\mathcal{H}_{\max}/\mathcal{H}_0)}
 \right],
 \qquad
 \mathcal{H}_{\max}=\max_{j,t}|\mathcal{H}_j(t)|.
 \label{eq:asinh_color_mapping}
\end{equation}
...
\begin{figure*}[t]
\vspace*{0.3in}
\includegraphics[width=\linewidth]{figure/fig04_p1_aplus.pdf}
\iffalse
\caption{Wave packet dynamics for the black-hole profile with $p=1$. System parameters are the same as in Fig.~\ref{fig:bh_p0}: in (a,b), $x_0=2\ell/3$ and $j_h\approx213$; in (c,d), $x_0=\ell/3$ and $j_h\approx113$. (a) Packet initialized outside the horizon with ingoing group velocity ($j_0=60$, $\sigma=0.05L$). The symmetric asinh color mapping in Eq.~\eqref{eq:asinh_color_mapping} is used with $\mathcal{H}_0=1$. (b) Packet initialized outside the horizon with outgoing group velocity ($j_0=120$, $\sigma=0.05L$). (c) Packet initialized inside the horizon with a right-moving initial spinor ($j_0=150$, $\sigma=0.05L$). (d) Packet initialized inside the horizon with a left-moving initial spinor ($j_0=150$, $\sigma=0.05L$). Black dotted lines indicate the event horizon, and cyan curves indicate the corresponding null geodesics $dx/dt=\beta(x)\pm1$. Interior oscillations in (c,d) are more pronounced than in the $p=0$ case owing to the stronger excitation of doubler modes, as confirmed by the $k$-space analysis of Fig.~\ref{fig:doubler_fft}.}
\label{fig:bh_p1}
\fi
\caption{\textit{Layout preview only: final caption to be supplied by the authors.}}
\label{fig:bh_p1}
\end{figure*}
\begin{figure*}[t]
\includegraphics[width=\linewidth]{figure/fig05_p1_aminus.pdf}
\caption{\textit{Layout preview only: final caption to be supplied by the authors.}}
...
(b) Spatial FFT magnitude of the mean-subtracted branch-resolved profile $\delta\mathcal{E}_{j,+}$ in Fig.~\ref{fig:bh_p1}(a) at successive times $t=0,3,3.5,4$, normalized by the maximum over the four spectra.
...
The secondary peak is already present before substantial boundary overlap, supporting its identification with doubler-mode excitation; its late-time amplitude at $t=3.5$ and $4$ can also be enhanced by contact with the right boundary.}
...
Here, the wave-packet width $\sigma(t)$ is calculated as the standard deviation of the spatial profile. Numerically, the nonnegative weights are obtained from the absolute value of the vacuum-subtracted $\mathcal{H}_j^-$ profile, and the width in lattice-site units is converted to physical length by multiplication by $\varepsilon=\ell/L$. We define $\delta\sigma(t)=\sigma(t)-\sigma(0)$.
