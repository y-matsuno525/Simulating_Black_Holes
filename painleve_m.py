...
def generate_BdG_matrix():
    ...
    def p(j):
        if p_const:
            ...
            return 1 - beta.beta(j,L,pos,epsilon)
    def dif_x_p(j):
        ...
        for j in range(2*L):
            ...
            if i < L and j < L:
                if i == j:
                    ...
                    H[i, j] = -1/(2*epsilon) * (p(i) - 1j*beta.beta(i,L,pos,epsilon))
                elif j-i == 1:
                    H[i, j] = -1/(2*epsilon) * (p(i) + 1j*beta.beta(i,L,pos,epsilon))
            #右上
            elif i < L and j >= L:
            ...
            #右下
                ...
                if i == j:
                    ...
                    H[i, j] = -1/(2*epsilon) * (-p(i-L) - 1j*beta.beta(i-L,L,pos,epsilon))
                elif j-i == 1:
                    H[i, j] = -1/(2*epsilon) * (-p(j-L) + 1j*beta.beta(j-L,L,pos,epsilon))
    if PBC:
        #周期境界条件
        #orange
    ...
    for j in range(L):
            for n in range(L):
                if pos == "ur" or pos == "lr":
                    ...
                    (1 - 1j)*eigenvectors[j, n].conj() + (1 + 1j)*eigenvectors[j, n+L]
                    #(1 - 1j)*eigenvectors[j, n].conj() + (1 + 1j)*eigenvectors[j, n+L]
                )
                else:
                    psi_tmp[n, 0] += weights[j] * (
...
for i in range(L):
    ...
    product = 0
    for j in range(L):
        product += eigenvectors[j+L,i] * eigenvectors[j,(2*L-1)-i]
        product += eigenvectors[j,i] * eigenvectors[j+L,(2*L-1)-i]
    print("productの大きさ")
    print(abs(product))
    print("productの値")
    print(product)
...
# #a+から移植
# cj_dag_cj1_dag_list = my_operator.generate_cj_dag_cj1_dag(eigenvectors,L)
# cj_dag_cj1_list = my_operator.generate_cj_dag_cj1(eigenvectors,L)
# cj_cj_list = my_operator.generate_cj_cj(eigenvectors,L)
# H_ps, H_ms = my_operator.H_p_m_M(cj_dag_cj1_dag_list, cj_dag_cj1_list, L, epsilon, pos)
# H_pms = my_operator.H_pm(cj_dag_cj1_dag_list, cj_dag_cj1_list, cj_cj_list, c_dag_c_list, L, epsilon, p_val, BH, PBC, l, pos)
...
# Hp_v, Hm_v, Hpm_v = my_operator.H_vacuum(eigenvectors,L, epsilon,p_val, pos)
...
#     for j, H_p in enumerate(H_ps):
#         val = np.conj(psi.T) @ H_p @ psi
#         current_H_p.append(val.item() - Hp_v[j])
#     H_p_list.append(current_H_p)

#     #H_mの期待値
#     for j, H_m in enumerate(H_ms):
#         val = np.conj(psi.T) @ H_m @ psi
#         current_H_m.append(val.item() - Hm_v[j])
